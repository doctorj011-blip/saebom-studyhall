...
REM ============================================================
REM  하는 일
REM   1) 인터넷이 되면 최신 타종 페이지를 받아 갱신한다 (안 되면 있던 파일로 진행)
REM   2) 크롬을 '타종 전용' 창으로 띄운다
REM
REM  아래 두 옵션이 이 파일의 핵심이다. 둘 다 빼면 안 된다.
REM
...
REM       탭만 하나 더 열리기 때문이다.
REM ============================================================

set "DIR=%~dp0"
set "PAGE=%DIR%saebom_bell.html"
set "TMP=%DIR%_bell.tmp"
...
curl -fsSL --connect-timeout 5 --max-time 20 -o "%TMP%" "%SRC%" >nul 2>&1
if not exist "%TMP%" goto NODL

REM 받은 게 진짜 타종 페이지인지 확인한다. 공유기 로그인 화면 같은 게 대신
REM 내려오는 일이 있어서, 멀쩡한 파일을 엉뚱한 것으로 덮어쓰면 안 된다.
findstr /c:"saebomBellSpeaker" "%TMP%" >nul 2>&1
if errorlevel 1 goto BADDL
move /y "%TMP%" "%PAGE%" >nul
echo         최신본으로 갱신했습니다.
goto HAVEPAGE
...
:NODL
echo         인터넷 연결이 없습니다. 컴퓨터에 있는 파일로 진행합니다.

:HAVEPAGE
if exist "%PAGE%" goto FINDBROWSER
echo.
